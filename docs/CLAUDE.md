...
src/
  ...
  dedup.py                 # 3-layer dedup: URL, fuzzy (company+title+state), cross-source + fuzzy-key history helpers
...
data/
  ...
  reviewed_fkeys.json      # Fuzzy-key index {company|title|state: {url, timestamp}} — blocks repost re-entry
...
- **Reviewed-job preservation**: Reviewed jobs get +1000 richness boost in dedup so they always survive when duplicates are merged. Additionally, every "Mark as Reviewed" action writes a `company|title|state` fuzzy key to `reviewed_fkeys.json`; the merge pipeline drops any new scraped row matching a reviewed fkey (different URL, same job) before it can re-enter master CSV. This prevents reposted LinkedIn/Indeed listings from resurfacing after review.
