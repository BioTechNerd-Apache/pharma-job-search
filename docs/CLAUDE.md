...
src/
  ...
  dashboard.py             # Streamlit UI with AG Grid — 3 tabs: Job Listings + Evaluations + Setup/Profile
  resume_parser.py         # Resume text extraction (PDF/DOCX/TXT) for setup wizard
  setup_wizard.py          # AI-powered setup wizard (3 Claude calls from resume)
...
- Setup wizard: `--setup resume.pdf` extracts resume text, makes 3 Claude Haiku calls to generate resume profile, search config, and evaluator patterns (~$0.01 total)

## Dependencies
python-jobspy, pandas, openpyxl, streamlit, streamlit-aggrid, pyyaml, requests, beautifulsoup4, anthropic, pdfplumber, python-docx
